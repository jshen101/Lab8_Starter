describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    await btn.click();
    const it = await btn.getProperty('innerText');
    // Once you have the innerText property, use innerText.jsonValue() to get the text value of it
    const text = await it.jsonValue();
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const prodItems = await page.$$('product-item');
    for (let i = 0; i < prodItems.length; i++) {
      // then for every single product element get the shadowRoot and query select the button inside, 
      // and click on it.
      const shadow = await prodItems[i].getProperty('shadowRoot');
      const btn = await shadow.$('button');
      if (await btn.evaluate((btn) => btn.innerText == 'Add to Cart')) {
        await btn.click();
      }
    }
    // Check to see if the innerText of #cart-count is 20
    const cartCount = await page.$('#cart-count');
    const it = await cartCount.getProperty('innerText');
    const text = await it.jsonValue();
    expect(text).toBe('20');
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      const shadow = await prodItems[i].getProperty('shadowRoot');
      const btn = await shadow.$('button');
      if (await btn.evaluate((btn) => btn.innerText == 'Remove from Cart')) {
        await btn.click();
      }
    }
    const cartCount = await page.$('#cart-count');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    await page.reload();
    // then go through each <product-item> to make sure that it has remembered nothing
